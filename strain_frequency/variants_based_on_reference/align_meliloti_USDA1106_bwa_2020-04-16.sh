#!/bin/bash
#
# Use BWA mem to align all the 33 extra strains sequenced at U Illinois to
# USDA1106.
...
INDIR="${PROJDIR}/results/clean_reads/illinois/trimgalore_htstream/2020-04-15_dedup_phix_trim"
OUTDIR="${PROJDIR}/results/align/${SPECIES}/${REFSTRAIN}/bwa/2020-04-16"
...
SCRATCHDIR="/scratch.global/${USER}/align_jgi_${SPECIES}_${REFSTRAIN}_bwa_2020-04-16"
...
if [[ "$PBS_ENVIRONMENT" == "PBS_BATCH" ]]; then
    ...
    for strain in "${INDIR}/"*; do 
        if [[ ! "$strain" =~ "work" && ! "$strain" =~ "script" && ! "$strain" =~ "log" && ! "$strain" =~ "array" ]]; then
