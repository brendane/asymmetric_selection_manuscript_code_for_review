...
permute_genotypes=''
permute_genotypes+='x=read.csv(file("stdin"),header=TRUE,sep="\t"); '
permute_genotypes+='x[, 2:ncol(x)] = x[, sample(2:ncol(x),ncol(x)-1,FALSE)]; '
permute_genotypes+='write.table(x, file=stdout(), sep="\t", quote=FALSE, col.names=TRUE, row.names=FALSE); '
...
OUTDIR="${PROJDIR}/results/genome_scan/spanfran2/allele_specific_w/2021-04-07_randomized"
SCRIPTDIR="${OUTDIR}/script_copies"
ARRAYJOBDIR="${OUTDIR}/arrayjobdata"
LOGDIR="${OUTDIR}/log"
WORKDIR="${OUTDIR}/working"
SCRATCHDIR="/scratch.global/${USER}/spanfran2_allele_specific_w_2021-04-07_randomized"
...
if [[ "$PBS_ENVIRONMENT" == "PBS_BATCH" ]]; then
    ...
    case "$TASK" in
        ...
        allele-frequency)
            ...
            # Reformat and randomize strain frequency table
            sed 's/SP2_//g' "$STRAIN_FREQUENCIES" \
                | Rscript -e "$permute_genotypes" \
                > final_freqs.tsv \
                || { echo "reformatting strain frequency and permuting genotypes header failed"; exit 1; }
        ...
        combine)
            ...
            "$hist_combiner" histogram.log2.*.tsv > combined_histogram.log2.tsv \
                || { echo "combining histograms failed"; exit 1; }
            cp combined_histogram.* "${OUTDIR}/${RUN}"
            ;;
        ...
        annotate)
            ...
            cut -f 2,15 top_candidates.tsv \
                ...
                | bedtools intersect -loj -a - -b "$MTR_ANNOTATION" -wb -nonamecheck \
            ...
            cp final_freqs.tsv combined_annotation.top.tsv ld_groups.txt ensifer_candidates.tsv ensifer_annotation.top.tsv medicago_annotation.top.tsv top_candidates*.tsv random_pairs.tsv "$OUTDIR/$RUN"
            ;;
