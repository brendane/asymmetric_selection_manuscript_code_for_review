#!/bin/bash
#
# Run GEMMA's univariate LMM on some nodule morphology traits and Shannon's
# diversity index # from the winter 2020 (SpanFran2) experiment.
#
#
# SETTINGS
#
...
TRAITS=( nod_number nod_mass nod_area shannon )
...
PHENOFILE="${PROJDIR}/results/phenotypes/gwas_phenotypes/spanfran2/2021-06-29_nod_shannon/phenotypes.tsv"
...
OUTDIR="${PROJDIR}/results/genome_scan/spanfran2/gwas/host/2021-06-29_nod_traits_lmm"
...
if [[ "$PBS_ENVIRONMENT" == "PBS_BATCH" ]]; then
    ...
    case "$TASK" in
        ...
        gwas-permuted)
            ...
            awk -F$'\t' 'NR == 1 {print "run\t" $2 "\t" $8 "\t" $14}; NR > 1 {print "real\t" $2 "\t" $4 "\t" $14};' \
            ...
            ## For now, skip running the permutations
...
#            tail -n +2 "${OUTDIR}/${PHENOTYPE}/output/output.real.effects.assoc.txt" \
#                | sort -k 14g,14 -t$'\t' \
#                | awk -F$'\t' 'NR < 101 { print $1 "\t" $3-1 "\t" $3 "\t" $2 ":" $14 };' \
#                > tmp.bed \
#                || { echo "making tmp bed file for ${PHENOTYPE} failed"; exit 1; }
#            bedtools window -w 10000 -a tmp.bed -b "$ANNOTATION" \
